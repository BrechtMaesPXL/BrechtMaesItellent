<script lang="ts">
  ...
  const activities = [
    ...
    },
    {
      category: "Seminars",
      title:
        "IBM: Discover the meeting point of GenAI, Prompt Engineering and Quantum Computing",
      description:
    ...
    },
    {
      ...
      description:
        "Tijdens deze hackathon werkten we in een team van twee personen aan een intensieve technische case rond Linux-serverbeheer en database-architectuur. We behaalden de 3de plaats.",
      icon: Trophy,
      color: "text-yellow-600",
      bgColor: "bg-yellow-100",
    ...
    },
    {
      ...
      description:
        "Innovatiewedstrijd gericht op het pitchen van een business concept. Eerste plaats behaald met een Virtual Reality oplossing voor de campus.",
      icon: Trophy,
      color: "text-yellow-600",
      bgColor: "bg-yellow-100",
    ...
    },
    {
      ...
      description:
        "Competitieve programmeerwedstrijd gericht op snelheid en efficiëntie. Tweede plaats behaald.",
      icon: Trophy,
      color: "text-yellow-600",
      bgColor: "bg-yellow-100",
      images: ["/pictures/image copy.png"],
    },
    {
      ...
      description:
        "Thema-hackathon over ruimtevaart-gerelateerde IT-uitdagingen. 4de plaats behaald.",
      icon: Trophy,
      color: "text-yellow-600",
      bgColor: "bg-yellow-100",
      images: ["/pictures/image copy 2.png"],
    ...
    },
    {
      ...
      description:
        "Buitenlandse stage-ervaring waarbij ik werk aan IT-projecten in een internationale context. Ontwikkeling van eLukulauta, een offline-first React Native app voor rendierhouderij met geavanceerde SQLite encryptie en P2P synchronisatie.",
      icon: Globe,
      color: "text-blue-600",
      bgColor: "bg-blue-100",
      images: [
        "/pictures/IMG20260402225219.jpg",
        "/pictures/IMG20260306175337.jpg",
      ],
    },
  ];
  ...
  <div class="flex flex-col md:flex-row justify-between items-end gap-6">
    <div>
      ...
      <p class="text-slate-600">
        A track record of continuous learning and achievement.
      </p>
    </div>
    ...
    {#each filteredActivities as activity}
      ...
      >
        <div class="shrink-0 mt-1">
          <div
            class="w-12 h-12 rounded-xl {activity.bgColor} {activity.color} flex items-center justify-center"
          >
            <Icon class="w-6 h-6" />
          </div>
        </div>
        <div>
          <span
            class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1 block"
          >
            {activity.category}
          </span>
          <h3
            class="text-xl font-bold text-slate-900 mb-2 group-hover:text-cyan-600 transition-colors"
          >
            {activity.title}
          </h3>
...
{#if selectedActivity}
  ...
  >
    <div
      class="glass-panel w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl p-6 shadow-2xl shadow-black/20"
      onclick={(e) => e.stopPropagation()}
    >
      <div class="flex justify-between items-start mb-6">
        <div>
          <span
            class="text-sm font-semibold text-slate-500 uppercase tracking-wider block mb-2"
            >{selectedActivity.category}</span
          >
        ...
        <button
          onclick={closeModal}
          class="p-2 rounded-full hover:bg-black/5 text-slate-500 hover:text-slate-900 transition-colors"
        >
          <X class="w-6 h-6" />
        </button>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {#each selectedActivity.images as img}
            <img
              src={img}
              alt="Activity"
              class="w-full h-auto rounded-xl object-cover aspect-video shadow-md border border-black/5"
            />
          {/each}
        </div>
      {/if}
